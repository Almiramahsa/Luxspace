  ...
  return (
    <div className="px-4 w-full md:w-2/12 mb-4 md:mb-0 accordion">
      <h5 className="text-lg font-semibold mb-2 relative">
        {title}
        <button onClick={() => setActive(isActive)} classname={['absolute block md:hidden right-0 transform -translate-y-1/2 focus:outline-none transition duration-200 top-1/2', isActive ? 'rotate-0' : 'rotate-180']}>
      ...
      <ul className={['md:h-auto md:visible md:opacity-100 overflow-hidden transition duration-200', isActive ? 'h-0 invisible opacity-0' : 'opacity-100']}>{children}</ul>
    </div>
  );
}
      ...
      <div className=" border-b border-gray-200 py-12 mt-16 px-4">
        ...
        <aside className="container mx-auto">
          <div className="flex flex-wrap -mx-4 justify-center">
            ...
            <FooterLinks setActive={setActive} isActive={active === 2 ? false : 2} title="Company">
              <li>
